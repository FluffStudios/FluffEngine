...
NK_STRTOD
You can define this to `strtod` or your own string to float conversion
...
NK_DTOA
You can define this to `dtoa` or your own float to string conversion
implementation replacement. If not defined nuklear will use its own
imprecise and possibly unsafe version (does not handle nan or infinity!).
<!> If used it is only required to be defined for the implementation part <!>
...
#define NK_POINTER_TYPE unsigned long
	...
	*  ForEach takes in a font handle and a specific way of handling memory. Memory control
	...
	*  nk_set_user_data    - Utility Functions_ to pass user data to draw command
	*/
	...
	*  nk_input_end        - Ends the input mirroring process by calculating state changes. Don't call any `nk_input_xxx` Functions_ referenced above after this call
	*/
	enum nk_keys {
		NK_KEY_NONE,
		NK_KEY_SHIFT,
	...
	*  This is basically a helper Functions_ to quickly push ASCII characters into
	...
	*  ForEach frame therefore fills the command buffer with draw commands
...
#define nk_draw_foreach(cmd,ctx, b) for((cmd)=nk__draw_begin(ctx, b); (cmd)!=0; (cmd)=nk__draw_next(cmd, b, ctx))
	...
	* All widgets inside nuklear can only be added inside Functions_ pair `nk_begin_xxx`
	* and `nk_end`. Calling any widgets outside these two functions will result in an
	* assert in debug or no state change in release mode.
	*
	* ForEach window holds frame persistent state like position, size, flags, state tables,
	* and some garbage collected internal persistent widget state. ForEach window
	...
	*      returns a `nk_window` struct pointing to the identified window or 0 if no window with given name was found */
	...
	/*  nk_window_get_bounds - returns a rectangle with screen position and size of the currently processed window.
	*  IMPORTANT: only call this Functions_ between calls `nk_begin_xxx` and `nk_end`
	*  Parameters:
	*      @ctx must point to an previously initialized `nk_context` struct
	*  Return values:
	...
	*
	*  To actually define a layout you just call the appropriate layouting Functions_
	...
	*  The easiest layouting Functions_ is `nk_layout_row_dynamic`. It provides each
	...
	*  Another easy layouting Functions_ is `nk_layout_row_static`. It provides each
...
#define nk_tree_image_push_id(ctx, type, img, title, state, id) nk_tree_image_push_hashed(ctx, type, img, title, state, NK_FILE_LINE,nk_strlen(NK_FILE_LINE),id)
	...
	* ============================================================================= */
	...
	NK_API void nk_property_double(struct nk_context*, const char *name, float min, float *val, float max, float step, float inc_per_pixel);
	NK_API int nk_propertyi(struct nk_context*, const char *name, int min, int val, int max, int step, float inc_per_pixel);
	NK_API float nk_propertyf(struct nk_context*, const char *name, float min, float val, float max, float step, float inc_per_pixel);
	NK_API float nk_propertyd(struct nk_context*, const char *name, float min, float val, float max, float step, float inc_per_pixel);
	/* =============================================================================
	*
	*                                  TEXT EDIT
	*
	...
	* ============================================================================= */
	...
	NK_API void nk_color_d(float *r, float *g, float *b, float *a, struct nk_color);
	NK_API void nk_color_dv(float *rgba_out, struct nk_color);
	...
	* ============================================================================= */
	...
	NK_API float nk_strtod(const char *str, const char **endptr);
	...
	*
	* ===============================================================*/
	/*  A basic (float)-buffer with linear allocation and resetting as only
...
#define NK_WINDOW_MAX_NAME 64
	...
	* flags, colors, fonts and for button_behavior. ForEach has it's own fixed size stack
