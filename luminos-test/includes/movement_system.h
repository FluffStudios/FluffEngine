...
struct MovementSystem : public ecs::System<MovementSystem>
{
	ecs::Entity Cam;
	glm::vec3 Position;
	glm::vec3 Rotation;
	...
	void FixedUpdate(ecs::EntityManager & Entities, ecs::EventManager & Events) override
	{
		...
		if (luminos::Keyboard::IsKeyPressed(KEY_W))
		{
		...
		if (luminos::Keyboard::IsKeyPressed(KEY_S))
		{
			const auto rot = Cam.GetComponent<gfx::CameraComponent>()->GetRotation();
			auto x = -sin(glm::radians(rot.y));
			auto y = -sin(glm::radians(rot.x));
		...
		if (luminos::Keyboard::IsKeyPressed(KEY_A))
		{
			const auto rot = Cam.GetComponent<gfx::CameraComponent>()->GetRotation();
		...
		if (luminos::Keyboard::IsKeyPressed(KEY_D))
		{
			const auto rot = Cam.GetComponent<gfx::CameraComponent>()->GetRotation();
			auto x = -cos(glm::radians(rot.y));
			auto z = sin(glm::radians(rot.y));
		...
		if (luminos::Keyboard::IsKeyPressed(KEY_PAGE_UP))
		{
			Velocity = glm::vec3(0, MOVEMENT_SPEED, 0);
		}

		if (luminos::Keyboard::IsKeyPressed(KEY_PAGE_DOWN))
		{
			Velocity = glm::vec3(0, -MOVEMENT_SPEED, 0);
		}

		if (luminos::Keyboard::IsKeyPressed(KEY_UP))
		{
			AngularVelocity = glm::vec3(LOOK_SPEED, 0, 0);
		}

		if (luminos::Keyboard::IsKeyPressed(KEY_DOWN))
		{
			AngularVelocity = glm::vec3(-LOOK_SPEED, 0, 0);
		}

		if (luminos::Keyboard::IsKeyPressed(KEY_RIGHT))
		{
			AngularVelocity = glm::vec3(0, -LOOK_SPEED, 0);
		}
		if (luminos::Keyboard::IsKeyPressed(KEY_LEFT))
		{
			AngularVelocity = glm::vec3(0, LOOK_SPEED, 0);
		}
	}
