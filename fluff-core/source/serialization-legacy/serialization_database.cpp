...
namespace fluff { namespace utilities
{
	...
	{
		if (locations.find(obj.name) != locations.end()) FLUFF_ERROR_FUNC("Item with provided name is already in database!")
	...
	{
		if (locations.find(mod.Name) != locations.end()) FLUFF_ERROR_FUNC("Item with provided name is already in database!")
