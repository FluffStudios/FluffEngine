...
namespace fluff { namespace utilities
{
	...
	std::vector<SerializationMesh> ModelLoader::LoadMesh(const aiScene* scene)
	{
		...
		for (size_t i = 0; i < scene->mNumMeshes; i++)
		{
			...
			for (size_t j = 0; j < static_cast<size_t>(m->mNumVertices); j++)
			{
				if (mesh.HasVertices)
				{
				...
				if (mesh.HasDiffuse)
				{
					for (size_t k = 0; k < static_cast<size_t>(m->GetNumColorChannels()); k++)
					{
						diffuse[3 * j + k] = m->mColors[0][uint32_t(j)][uint32_t(k)];
					}
				...
				if (mesh.HasSpecular)
				{
					for (size_t k = 0; k < static_cast<size_t>(m->GetNumColorChannels()); k++)
					{
						specular[3 * j + k] = m->mColors[1][uint32_t(j)][uint32_t(k)];
					}
					if (m->GetNumColorChannels() == 3)
					{
						specular[3 * j + size_t(3)] = 1.0f;
					}
				}
			}
