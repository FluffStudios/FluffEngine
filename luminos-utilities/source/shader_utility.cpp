...
namespace luminos { namespace utilities
{
	std::string ShaderUtility::LoadShader(std::string FileName, std::unordered_map<std::string, unsigned> Uniforms)
	{
			...
			while(input.good())
			{
				...
				if (strcmp(temp.c_str(), "#include") == 0)
				{
					if (input.good())
					{
						...
						{
							if (begin != '<')
								LUMINOS_SHADER_ERROR_FUNC(FileName.c_str(), line, "Invalid Syntax.  Expected \"<\" before file name.")
							if (end != '>')
								LUMINOS_SHADER_ERROR_FUNC(FileName.c_str(), line, "Invalid Syntax.  Expected \">\" after file name.")
						}
					}
					else
					{
						LUMINOS_SHADER_ERROR_FUNC(FileName.c_str(), line, "File ended before #include could complete.")
					}
				}
				else
				{
		...
		{
			LUMINOS_ERROR_FUNC("Could not open file.");
		}
